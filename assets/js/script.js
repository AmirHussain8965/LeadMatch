...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (submitBtn) {
        submitBtn.addEventListener("click", function () {
            ...
            [fullName, businessEmail, businessType, monthlyRevenue].forEach(input => {
                if (input) input.style.borderColor = "transparent";
            });

            // Validate fields
            if (!fullName || fullName.value.trim() === "") {
                if (fullName) fullName.style.borderColor = "#e74c3c";
                isValid = false;
            }

            if (!businessEmail || !isValidEmail(businessEmail.value.trim())) {
                if (businessEmail) businessEmail.style.borderColor = "#e74c3c";
                isValid = false;
            }

            if (!businessType || businessType.value === "") {
                if (businessType) businessType.style.borderColor = "#e74c3c";
                isValid = false;
            }

            if (!monthlyRevenue || monthlyRevenue.value.trim() === "") {
                if (monthlyRevenue) monthlyRevenue.style.borderColor = "#e74c3c";
                isValid = false;
            }

            if (!isValid) {
                // Shake animation on invalid
                const formCard = document.querySelector(".apply_form_card");
                if (formCard) {
            ...
            const btnLoader = submitBtn.querySelector(".btn_loader");
